...
# --- SHARED CONFIG ---
KEYWORDS = {
    'funding': ['funding', 'raised', 'valuation', 'investment', 'capital', 'series', 'round'],
    'crisis': ['layoff', 'debt', 'loss', 'shutdown', 'crisis', 'distressed'],
    'growth': ['expansion', 'launch', 'revenue', 'growth', 'profit', 'partnership']
}
...
<style>
    ...
    [data-testid="stSidebar"] {{ background-color: {OLIVE} !important; }}
...
def get_live_features(df_l, tfidf, feature_cols):
    ...
    mat = tfidf.transform(df_l['cl'])
    sem = pd.DataFrame(mat.toarray(), columns=tfidf.get_feature_names_out()).mean()
    s = df_l['sentiment_score'].values; t = df_l['cl'].values; n = len(s)
    sig = {
        'mean_sent': np.mean(s), 'std_sent': np.std(s) if n > 1 else 0,
        'pos_rate': (s > 0.05).mean(),
        'funding_signal': sum(1 for x in t if any(w in x for w in KEYWORDS['funding']))/n,
        'crisis_signal': sum(1 for x in t if any(w in x for w in KEYWORDS['crisis']))/n,
        'growth_signal': sum(1 for x in t if any(w in t for w in KEYWORDS['growth']))/n
    }
    return pd.DataFrame([pd.concat([sem, pd.Series(sig)])])[feature_cols]
