...
if page == "Strategic Summary":
    ...
    if target:
        with st.spinner(f"Decoding {target}..."):
            ...
            if recs:
                ...
                p_r_raw = le_raw.inverse_transform(m_raw.predict(l_f))[0]
                p_s_raw = le_stg.inverse_transform(m_stg.predict(l_f))[0]
                # Display-friendly labels for funding series
                series_display = {'Growth Stage': 'Active Funding (Pre-IPO)', 'Listed / Public': 'Listed / Public', 'Distressed': 'Distressed / Shut Down'}
                stage_display = {'Growth': 'Growth Phase', 'Public': 'Public', 'Distressed': 'Distressed'}
                p_r = series_display.get(p_r_raw, p_r_raw)
                p_s = stage_display.get(p_s_raw, p_s_raw)
                avg_conf = (metrics['series']['accuracy'] + metrics['stage']['accuracy']) / 2
                st.write("---")
                c1, c2, c3 = st.columns(3)
                with c1: st.markdown(f'<div class="bg-white p-8 rounded-2xl shadow-xl border-b-[6px] border-[{MAGENTA}] text-center"><p class="text-sm uppercase text-gray-400 font-bold tracking-widest">Funding Series</p><h2 class="text-3xl font-black text-[{MAGENTA}] mt-2">{p_r}</h2></div>', unsafe_allow_html=True)
                with c2: st.markdown(f'<div class="bg-white p-8 rounded-2xl shadow-xl border-b-[6px] border-[{OLIVE}] text-center"><p class="text-sm uppercase text-gray-400 font-bold tracking-widest">Startup Stage</p><h2 class="text-3xl font-black text-[{OLIVE}] mt-2">{p_s}</h2></div>', unsafe_allow_html=True)
                with c3: st.markdown(f'<div class="bg-white p-8 rounded-2xl shadow-xl border-b-[6px] border-gray-100 text-center"><p class="text-sm uppercase text-gray-400 font-bold tracking-widest">Confidence Level</p><h2 class="text-3xl font-black mt-2">{avg_conf*100:.1f}%</h2></div>', unsafe_allow_html=True)
                
                st.markdown(f"""<div class="bg-white p-10 rounded-3xl shadow-2xl mt-10 border border-gray-50 flex items-center gap-4"><i data-lucide="check-circle" class="w-10 h-10 text-[{SUCCESS}]"></i><p class="text-2xl font-light text-gray-700 leading-relaxed">Identity Confirmed: <b>{target}</b> is identified as <b>{p_r}</b> (funding) in <b>{p_s}</b> (stage).</p></div>""", unsafe_allow_html=True)
    ...
    # Name clusters based on dominant startup characteristics
    startup_names = list(df_hist.groupby('startup').groups.keys())
    cluster_labels = {}
    for c_id in range(4):
        members = [startup_names[i] for i in range(len(clusters)) if clusters[i] == c_id]
        stages = [df_hist[df_hist['startup'] == m]['startup_stage'].iloc[0] if 'startup_stage' in df_hist.columns else '' for m in members]
        if any('Public' in str(s) for s in stages) and sum('Public' in str(s) for s in stages) >= len(members)//2:
            cluster_labels[c_id] = 'Public / Listed Ventures'
        elif any('Shut Down' in str(s) or 'Insolvency' in str(s) for s in stages):
            if sum('Shut Down' in str(s) or 'Insolvency' in str(s) for s in stages) >= len(members)//2:
                cluster_labels[c_id] = 'Distressed Ventures'
            else:
                cluster_labels[c_id] = 'Mixed Portfolio'
        else:
            # Check sentiment
            avg_sent = np.mean([df_hist[df_hist['startup'] == m]['sentiment_score'].mean() for m in members])
            if avg_sent > 0.15:
                cluster_labels[c_id] = 'High-Momentum Growth'
            else:
                cluster_labels[c_id] = 'Steady-State Growth'
    named_clusters = [cluster_labels.get(c, f'Cluster {c}') for c in clusters]
    # Name PCA dimensions based on feature loadings
    pca_var = pca.explained_variance_ratio_
    dim1_label = f'Narrative Theme ({pca_var[0]*100:.0f}% variance)'
    dim2_label = f'Sentiment Polarity ({pca_var[1]*100:.0f}% variance)'
    df_pca = pd.DataFrame({dim1_label: coords[:, 0], dim2_label: coords[:, 1], 'Cluster': named_clusters, 'Startup': startup_names})
    st.plotly_chart(px.scatter(df_pca, x=dim1_label, y=dim2_label, color='Cluster', hover_data=['Startup'], color_discrete_sequence=M_O_COLORS, title="Semantic Venture Clusters (PCA Projection)"), use_container_width=True)
    st.write("---")
